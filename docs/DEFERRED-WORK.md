...
Status: current after the authorized Phase 3 cohort closeout and the bounded
Phase 4 fact-check output-budget closeout on 2026-08-26.
...
| Fact-check output-budget compatibility | The initial F1-F4 `64`/`256` study is complete, including the owner-selected Outcome A closeout. The owner explicitly started the separately planned five-arm `64`/`96`/`128`/`192`/`256` breakpoint follow-up on 2026-08-26. Its one fresh 60-row execution from clean commit `0d4e009` verified and reanalyzed offline. Valid verdict yields were `2/12`, `2/12`, `2/12`, `6/12`, and `12/12` in token-arm order; completion-token counts remain only an output-limit proxy. The new ignored evidence does not reuse, alter, or repair the immutable pair. | Preserve all five saved arms. The owner may record a bounded interpretation of the verified aggregates; no agent selects it. No arm may be rerun, replaced, repaired, or published. |
...
| Embedding retrieval, answer generation, and relevancy evaluation | R0–R6 implementation is complete: the approved public corpus, confirmed query catalog, deterministic lexical baseline, saved retrieval-only evidence, opt-in local embedding/answer boundaries, and opt-in R6 evaluator boundary preserve the required identities and evidence lifecycle. R4 remains deferred because no installed local model currently advertises Ollama's `embedding` capability. R5 and R6 have no selected or invoked models and no formal evidence. R6 consumes one verified clean R5 run, gives `RelevancyEvaluator` only the retained documents, rejects missing context or unavailable answers, and preserves retrieval expectation, evaluator observation, self-evaluation, human support, and answer correctness separately. None of this establishes embedding quality, answer correctness, general semantic relevance, or an AI-evaluator judgment. | For an R4 vector run, first have an already-installed model advertising `embedding`, then lock its tag, full digest, chunking, normalization, distance, and top-K contract from a clean commit. For an R5 answer run, select one already-installed local answer model and use one verified clean R3 run to lock its full digest, prompt, temperature, seed, token limit, timeout, one-attempt/no-pull settings, and new dated output; then verify/reanalyze offline. For an R6 run, select one already-installed evaluator model and use one verified clean R5 run to lock its full digest, prompt, settings, and new dated output; then verify/reanalyze offline. Pulls, remote providers, credentials, spending, and Docker remain separately gated. |
