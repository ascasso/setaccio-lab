...
| Embedding retrieval, answer generation, and relevancy evaluation | R0–R3 are complete: the approved public corpus, confirmed query catalog, deterministic lexical baseline, and saved retrieval-only evidence now preserve complete returned document text with ranked identities. The provider-free baseline does not establish embedding quality, generated-answer quality, general semantic relevance, or an AI-evaluator judgment. | Explicitly request R4 and lock one already-installed local embedding model's tag, full digest, chunking, normalization, distance, and top-K contract before formal vector generation. R5 and R6 remain later; pulls, remote providers, credentials, spending, and Docker remain separately gated. |
