...
| Embedding retrieval, answer generation, and relevancy evaluation | R0–R6 implementation is complete: the approved public corpus, confirmed query catalog, deterministic lexical baseline, saved retrieval-only evidence, opt-in local embedding/answer boundaries, and opt-in R6 evaluator boundary preserve the required identities and evidence lifecycle. R4 remains deferred because no installed local model currently advertises Ollama's `embedding` capability. One R5 clean-baseline matrix completed on 2026-08-30 from commit `c724e5a93c89eb5de8a11e9d1774a523f77bda37`; one R6 clean-baseline matrix then completed from commit `f704d989429a10769ce334276dc79de5bd7cd308` against that verified R5 source. R6 used the operationally selected already-installed `granite4.1:3b` artifact and full digest, separate from R5's answer artifact. It retained all 14 rows and its ignored evidence verified and reanalyzed offline: eight eligible evaluator calls completed, two missing-context rows and four unavailable-answer rows were not attempted, and no unavailable-model, timeout, provider-failure, empty-response, or malformed-verdict outcome occurred. The relationship was recorded as separate evaluator for every row; it does not establish independence. Retrieval expectation, evaluator observation, self-evaluation, human support, and answer correctness remain separate. None of this establishes embedding quality, evaluator ground truth, answer correctness, semantic relevance, model quality, or ranking. | Preserve the ignored R5/R6 evidence; do not rerun, repair, replace, or publish raw answers or evaluator output. For an R4 vector run, first have an already-installed model advertising `embedding`, then lock its tag, full digest, chunking, normalization, distance, and top-K contract from a clean commit. Do not conduct human semantic review under this closeout. Pulls, remote providers, credentials, spending, and Docker remain separately gated. |
