...
| Fact-check output-budget compatibility | Completed on 2026-08-26 through F1-F3 implementation, one fresh paired `64`/`256` run from clean commit `b758dcb`, offline verification/reanalysis, deterministic F3 comparison, and the owner-selected Outcome A closeout. The installed `gemma4:e2b` judge retained `2/12` valid verdicts at `64` tokens and `12/12` at `256`; the ten empty `64`-token responses were all at the configured limit, while all `256`-token rows recorded completion-token counts below it. This is a narrow deployed-protocol output-budget observation, not a judge ranking, general factuality or reliability claim, or provider finish-reason finding. A5 remains contextual only. See [the F4 closeout](logs/2026-08-26-phase4-fact-check-output-budget.md). | No remaining Phase 4 gate. Preserve the immutable ignored pair; do not rerun, replace, repair, or publish raw outputs. Any `64`/`96`/`128`/`192`/`256` breakpoint study is separately planned and explicitly started work. |
