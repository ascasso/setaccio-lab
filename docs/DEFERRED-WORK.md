...
| Small-model compatibility cohort execution and T3.4 interpretation | Completed through the authorized clean-baseline run from commit `e897edf` on 2026-08-24. The owner-approved five peers plus separately labelled `qwen3.8:27b-mlx` reference ran once under Ollama `0.32.15`, the untreated policy, and the locked 96-row sequential protocol. All rows were retained in ignored evidence, which verifies and reanalyzes offline. The bounded public-safe T3.4 record is per-model, preserves incomplete/unsupported and mixed-runtime observations, and makes no rank, selection, general-capability, semantic-correctness, or backend-normalized performance claim. | Preserve the immutable evidence. T3.5 or any new cohort run requires separate explicit authorization; do not rerun, repair, replace, pull, substitute, or customize a model. |
