...
| Embedding retrieval, answer generation, and relevancy evaluation | R0–R4 implementation is complete: the approved public corpus, confirmed query catalog, deterministic lexical baseline, saved retrieval-only evidence, and opt-in local embedding boundary preserve the required identities and evidence lifecycle. The R4 formal vector run remains deferred because no installed local model currently advertises Ollama's `embedding` capability. Neither provider-free work nor an eventual R4 run establishes embedding quality, generated-answer quality, general semantic relevance, or an AI-evaluator judgment. | When an already-installed local embedding-capable model is available, lock its tag, full digest, chunking, normalization, distance, and top-K contract from a clean commit; then execute the one formal R4 run and verify/reanalyze it offline. R5 and R6 remain later; pulls, remote providers, credentials, spending, and Docker remain separately gated. |
