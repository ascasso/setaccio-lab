...
| Fact-check output-budget compatibility | The initial F1-F4 `64`/`256` study is complete, including the owner-selected Outcome A closeout. On 2026-08-26, the owner explicitly started its separately planned five-arm `64`/`96`/`128`/`192`/`256` breakpoint follow-up. The new study uses fresh ignored evidence and must not reuse, alter, or repair the immutable pair. | Implement and verify the isolated opt-in five-arm runner and offline lifecycle from a clean commit, then execute each fresh arm once under the locked protocol. Preserve all evidence and leave the aggregate interpretation to the owner. |
