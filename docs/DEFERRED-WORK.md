...
| Small-model compatibility cohort execution | Partially prepared. Provider-free ordered identity resolution, explicit metadata availability, local/runtime/digest/alias guards, failure-before-allocation behavior, and the four-outcome prompt-policy mapping are implemented. The T3.3 core now locks a dynamic model-major schedule, omits unsupported seeds, preserves effective-thinking metadata without overrides, isolates advisor state per model, retains complete rows/failures, writes strict shared-v1 evidence, and exposes offline verify/reanalyze tasks. All provisional tags were observed as installed under Ollama `0.32.15`, but the cohort and reference identity are not owner-approved or locked. No live cohort task or model invocation exists. | Complete and verify T2.5, approve the exact ordered peers and separately labelled reference with full digests, then complete the approved live-runner boundary and separately authorize one exact clean-baseline cohort command. |
