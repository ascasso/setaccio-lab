...
| Embedding retrieval, answer generation, and relevancy evaluation | R0–R6 implementation is complete: the approved public corpus, confirmed query catalog, deterministic lexical baseline, saved retrieval-only evidence, opt-in local embedding/answer boundaries, and opt-in R6 evaluator boundary preserve the required identities and evidence lifecycle. R4 remains deferred because no installed local model currently advertises Ollama's `embedding` capability. One R5 clean-baseline matrix completed on 2026-08-30 from commit `c724e5a93c89eb5de8a11e9d1774a523f77bda37`, using the operationally selected already-installed `gemma4:e2b` tag and its full digest against the verified R3 baseline. All 14 rows were retained and the ignored evidence verified and reanalyzed offline: 10 completed, four were empty responses, two used exact `NO_SUPPORT`, and no model-unavailable, timeout, authentication, rate-limit, or provider-failure outcome occurred. These are not answer-correctness, semantic-support, relevance, quality, or ranking findings. R6 remains unrun with no selected or invoked evaluator. It consumes one verified clean R5 run, gives `RelevancyEvaluator` only the retained documents, rejects missing context or unavailable answers, and preserves retrieval expectation, evaluator observation, self-evaluation, human support, and answer correctness separately. | Preserve the ignored R5 evidence; do not rerun, repair, replace, or publish its raw answers. For an R4 vector run, first have an already-installed model advertising `embedding`, then lock its tag, full digest, chunking, normalization, distance, and top-K contract from a clean commit. Do not begin R6 in this closeout; any later R6 work must select one already-installed evaluator model and use the verified clean R5 run to lock its full digest, prompt, settings, and new dated output, then verify/reanalyze offline. Pulls, remote providers, credentials, spending, and Docker remain separately gated. |
