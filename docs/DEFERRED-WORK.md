...
| Small-model compatibility cohort through T3.5 | Completed through the authorized clean-baseline run from commit `e897edf` on 2026-08-24, bounded T3.4 interpretation, and provider-free T3.5 implementation plus one deterministic offline comparison on 2026-08-25. The owner-approved five peers plus separately labelled `qwen3.8:27b-mlx` reference ran once under Ollama `0.32.15`, the untreated policy, and the locked 96-row sequential protocol. All rows remain in ignored verified evidence. T3.5 paired every peer row with the reference without provider access or evidence mutation and found reference-only pass counts of `16`, `2`, `4`, `2`, and `2` in peer order, with no peer-only or neither-pass rows. This is not a rank, correctness oracle, selection, general-capability, semantic-correctness, or backend-normalized performance result. | Preserve the immutable evidence. Optional T3.6 or any new cohort run requires separate explicit authorization; do not rerun, repair, replace, pull, substitute, or customize a model. |
