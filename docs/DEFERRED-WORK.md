...
| Embedding retrieval, answer generation, and relevancy evaluation | R0–R5 implementation is complete: the approved public corpus, confirmed query catalog, deterministic lexical baseline, saved retrieval-only evidence, opt-in local embedding boundary, and opt-in answer boundary preserve the required identities and evidence lifecycle. The R4 formal vector run remains deferred because no installed local model currently advertises Ollama's `embedding` capability. R5 has no selected or invoked answer model and no formal answer evidence. Its runner consumes one verified clean R3 run, retains every retrieved document/rank beside each answer, and leaves assertion support unassessed. None of this establishes embedding quality, answer correctness, general semantic relevance, or an AI-evaluator judgment. | For an R4 vector run, first have an already-installed model advertising `embedding`, then lock its tag, full digest, chunking, normalization, distance, and top-K contract from a clean commit. For an R5 answer run, select one already-installed local answer model and use one verified clean R3 run to lock its full digest, prompt, temperature, seed, token limit, timeout, one-attempt/no-pull settings, and new dated output; then verify/reanalyze offline. R6 remains later. Pulls, remote providers, credentials, spending, and Docker remain separately gated. |
