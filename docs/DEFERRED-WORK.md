...
| Fact-check output-budget compatibility | F1 and F2 implementation are complete as provider-free boundaries: the dedicated paired task fixes fresh `64` and `256` arms, and the pair evidence lifecycle writes, verifies, and deterministically reanalyzes two arm directories while requiring identical non-budget protocol fields. No formal arm was executed in these slices, and A5 remains contextual evidence only. F3 offline comparison and F4 interpretation remain deferred. | Begin the standing-authorized formal pair only after reviewing the F1/F2 tasks. Use two new ignored evidence directories from one clean Git commit; never reuse A5, salvage a changed-baseline arm, selectively retry a row, or replace evidence. Begin F3 only after both fresh arms verify as one pair. |
