...
| Fact-check output-budget compatibility | The initial F1-F4 `64`/`256` study is complete, including the owner-selected Outcome A closeout. The separately planned five-arm `64`/`96`/`128`/`192`/`256` breakpoint follow-up ran once from clean commit `0d4e009`, then verified and reanalyzed offline. The owner-reviewed interpretation records a flat valid-verdict yield of `2/12` at `64`–`128` tokens, with higher yields at `192` (`6/12`) and `256` (`12/12`) tokens. This is a protocol-specific association, not a causal threshold, generally optimal budget, model-reliability, factuality, or ranking claim; completion-token counts remain only an output-limit proxy. The new ignored evidence does not reuse, alter, or repair the immutable pair. | Preserve all five saved arms. No arm may be rerun, replaced, repaired, altered, or published. Any new experiment or interpretation rule requires separate explicit authorization. |
