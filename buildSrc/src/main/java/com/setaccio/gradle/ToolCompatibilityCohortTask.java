...
@DisableCachingByDefault(because = "Runs an explicit live local cohort and writes non-overwriting evidence")
public abstract class ToolCompatibilityCohortTask extends DefaultTask {

    private final ExecOperations execOperations;

    @Inject
    public ToolCompatibilityCohortTask(ExecOperations execOperations) {
        this.execOperations = execOperations;
        setGroup("verification");
        setDescription("Runs the explicit local-only, no-pull 96-row approved tool compatibility cohort; never part of the default lifecycle.");
    }
    ...
    private void requireOption(Property<String> property, String option) {
        ...
        if (value == null || value.isBlank()) {
            throw new GradleException("toolCompatibilityCohort requires " + option + "=<value>");
        }
    }
}
