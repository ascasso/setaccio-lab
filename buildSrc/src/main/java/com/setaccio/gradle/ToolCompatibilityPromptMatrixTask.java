...
/** Launches the one explicitly requested paired Phase 2 prompt-matrix process. */
@DisableCachingByDefault(because = "Runs an explicit live local model matrix and writes non-overwriting evidence")
public abstract class ToolCompatibilityPromptMatrixTask extends DefaultTask {

    private final ExecOperations execOperations;

    @Inject
    public ToolCompatibilityPromptMatrixTask(ExecOperations execOperations) {
        this.execOperations = execOperations;
        setGroup("verification");
        setDescription("Runs the explicit local-only, no-pull 32-row paired prompt matrix; never part of the default lifecycle.");
    }
    ...
    public void runMatrix() {
        ...
        requireOption(getBaselineOutputDir(), "--baseline-output-dir");
        requireOption(getCandidateOutputDir(), "--candidate-output-dir");
        execOperations.javaexec(spec -> {
            ...
            spec.args(List.of(
                    ...
                    "--baseline-output-dir", getBaselineOutputDir().get(),
                    "--candidate-output-dir", getCandidateOutputDir().get()));
        });
    }
    ...
    private void requireOption(Property<String> property, String option) {
        ...
        if (value == null || value.isBlank()) {
            throw new GradleException("toolCompatibilityPromptMatrix requires " + option + "=<value>");
        }
    }
}
