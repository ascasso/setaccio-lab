...
public final class ChatMatrixOfflineRunner {

    private ChatMatrixOfflineRunner() {}

    public static void main(String[] args) {
        Arguments parsed = Arguments.parse(args);
        Path runDirectory = parsed.mode() == Mode.REANALYZE
                ? ChatMatrixProtocol.EVIDENCE_ROOT.requireReanalyzableRunDirectory(
                        Path.of(""), parsed.runDirectory(), "Run directory")
                : resolveRunDirectory(parsed.runDirectory());
        ObjectMapper objectMapper = JsonMapper.builder().findAndAddModules().build();
        ChatMatrixEvidence evidence = new ChatMatrixEvidence(
                objectMapper,
                ChatPromptCatalog.load(objectMapper));
        ChatMatrixEvidence.OfflineResult result = parsed.mode() == Mode.VERIFY
        ...
        if (!result.valid()) {
            ...
            throw new IllegalStateException(
                    "Chat matrix evidence " + parsed.mode().label + " failed with "
                            + result.failures().size() + " issue(s).");
        }
        System.out.println("Chat matrix evidence " + parsed.mode().label + " complete: "
                + runDirectory.resolve(ChatMatrixEvidence.SUMMARY_FILENAME));
    }
