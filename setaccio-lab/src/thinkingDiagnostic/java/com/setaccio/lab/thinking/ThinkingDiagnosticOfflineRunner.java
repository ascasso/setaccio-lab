...
/** Verifies or deterministically regenerates saved diagnostic evidence, provider-free. */
public final class ThinkingDiagnosticOfflineRunner {

    private ThinkingDiagnosticOfflineRunner() {}

    public static void main(String[] args) {
        Arguments parsed = Arguments.parse(args);
        Path runDirectory = parsed.mode() == Mode.REANALYZE
                ? ThinkingDiagnosticProtocol.EVIDENCE_ROOT.requireReanalyzableRunDirectory(
                        Path.of(""), parsed.runDirectory(), "Run directory")
                : resolveRunDirectory(parsed.runDirectory());
        ObjectMapper objectMapper = JsonMapper.builder().findAndAddModules().build();
        ThinkingDiagnosticEvidence evidence = new ThinkingDiagnosticEvidence(
                objectMapper, new LocalFactCheckFixtureCatalog(objectMapper));
        ThinkingDiagnosticEvidence.OfflineResult result = parsed.mode() == Mode.VERIFY
        ...
        if (!result.valid()) {
            ...
            throw new IllegalStateException("Thinking diagnostic evidence " + parsed.mode().label
                    + " failed with " + result.failures().size() + " issue(s).");
        }
        System.out.println("Thinking diagnostic evidence " + parsed.mode().label + " complete: "
                + runDirectory.resolve(ThinkingDiagnosticEvidence.SUMMARY_FILENAME));
    }

    static Path resolveRunDirectory(String value) {
        return ThinkingDiagnosticProtocol.EVIDENCE_ROOT.requireSavedRunDirectory(
