...
- A paired clean Prompt v2 local matrix completed from commit `6b5b970` with
  ...
  compared against the immutable v1 run. Pre-registered human review found
  primary concepts retained in 11 of 12 model/case judgments and partially
  retained in one, with no total loss. Version 2 reduced some unsupported exact
  specificity but not uniformly; generic context and low-quality overconfidence
  persisted, so version 1 remains the interactive default.
